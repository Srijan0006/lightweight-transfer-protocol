...
public class Receiver {
    ...
    public static void main(String[] args) throws Exception {
        ...
        JFrame frame = new JFrame("LWSP Receiver Monitor");
        ...
        while (true) {
            ...
            System.out.printf("Received: Frame %d, Frag %d/%d%n",
                    packet.frameId, packet.fragIndex + 1, packet.totalFrags);

            // Safer memory eviction strategy
            int currentFrameId = packet.frameId;
            frameBuffer.keySet().removeIf(oldFrameId -> oldFrameId < currentFrameId - 2);

            frameBuffer.computeIfAbsent(packet.frameId, k -> new byte[packet.totalFrags][]);
            ...
            if (isComplete(frags)) {
                ...
                frameBuffer.remove(packet.frameId);

                BufferedImage img = ImageIO.read(new ByteArrayInputStream(frameBytes));
                if (img != null) {
                    Image scaled = img.getScaledInstance(frame.getWidth(), frame.getHeight(), Image.SCALE_FAST);
